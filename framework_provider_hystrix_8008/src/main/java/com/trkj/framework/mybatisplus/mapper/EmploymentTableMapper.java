...
public interface EmploymentTableMapper extends BaseMapper<EmploymentTable> {
     ...
     * @return
     */
    @Select("select s.STAFF_ID,s.STAFF_NAME,w.WORK_EXPERIENCE_ID,w.WORK_STARE_TIME,w.WORK_END_TIME,w.COMPANY_NAME,w.POSITION_NAME,w.POSITION_INDUSTRY,w.POSITION_DESCRIBE,w.POSITION_SQMONTHLY,g.GLORY_ID,g.GLORY_NAME,g.GLORY_UNITNAME,g.GLORY_REMARK,p.PUNISH_ID,p.PUNISH_TYPE,p.PUNISH_CAUSE,p.PUNISH_UNIT,p.IS_REVOCATION,p.PUNISH_REMARK,e.EDUCATION_ID,e.EDUCATION_START_TIME,e.EDUCATION_END_TIME,e.EDUCATION_STUDENTNAME,e.EDUCATION_MAJOR,e.EDUCATION_FULL_TIME FROM WORK_EXPERIENCE w LEFT JOIN STAFF s on w.STAFF_ID=s.STAFF_ID LEFT JOIN GLORY g on g.STAFF_ID=s.STAFF_ID LEFT JOIN PUNISH p on p.STAFF_ID=s.STAFF_ID LEFT JOIN EDUCATION e on e.STAFF_ID=s.STAFF_ID ${ew.customSqlSegment}")
