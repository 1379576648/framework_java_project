...
public class GlobalExceptionHandler {
    ...
    public AjaxResponse defaultErrorHandler(HttpServletRequest request, Exception e) {
        ...
        if (e instanceof NoHandlerFoundException) {
            log.info("没有找到匹配的url");
            return AjaxResponse.error(new com.trkj.framework.exception.CustomError(com.trkj.framework.exception.CustomErrorType.PAGE_NOT_FOUND_ERROR, e.getMessage()));
        } else if (e instanceof SignatureException) {
            log.info("签名验证异常");
            return AjaxResponse.error(new com.trkj.framework.exception.CustomError(com.trkj.framework.exception.CustomErrorType.SIGNATURE_ERROR, e.getMessage()));
        }
        log.info("系统产生了异常");
